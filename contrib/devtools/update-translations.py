...
def remove_current_translations():
    ...
    This leaves azus_en.ts untouched.
    '''
...
def update_build_systems():
    ...
    '''
    filename_lang = [re.match(r'((azus_(.*)).ts)$', filename).groups() for (filename, filepath) in all_ts_files(include_source=True)]
    filename_lang.sort(key=lambda x: x[0])

    # update qrc locales
    with open('src/qt/azus_locale.qrc', 'w') as f:
