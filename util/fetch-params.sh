...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/AZUSParams"
    else
        PARAMS_DIR="$HOME/.azus-params"
    fi
fi
...
Failed to fetch the AZUS zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
AZUS - fetch-params.sh

This script will fetch the AZUS zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common AZUS zkSNARK parameters. Note that it is
