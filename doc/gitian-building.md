...
*Setup instructions for a gitian build of AZUS Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the AZUS
...
to the AZUS GitHub release page.
...
- [Building AZUS Core](#building-azus-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/azus-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/azus-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/azus-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/azus-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/azus-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/azus-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/AzusNodes/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@azus.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/azus-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/azus-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/azus-osx-*-build.assert
```
