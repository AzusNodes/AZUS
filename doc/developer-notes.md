...
- [Development guidelines](#development-guidelines)
    - [General AZUS Core](#general-azus-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
AZUS Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play AZUs (tAZU)" on the test network, if you
...
AZUS Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/AzusNodes/AZUS/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_azus
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_azus --log_level=test_suite
$ valgrind -v --leak-check=full src/azusd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_azus.coverage/index.html`.
```
...
- BitcoinMiner : Generates AZUs (if wallet is enabled).
...
However, in open source software such as AZUS Core, where everyone uses
...
pay attention to for reviewers of AZUS Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: AZUS Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against AZUS Core so that they can be integrated
...
file descriptors it will cause problems with AZUS's `select()` loop, because
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `azus-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `azus-cli` and the GUI debug console use this table to determine how to
