...
from test_framework.test_framework import AzusTestFramework
...
class ReorgStakeTest(AzusTestFramework):
    ...
    def check_money_supply(self, expected_azu, expected_zazu):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected AZU and zAZU supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_azu))
            for denom in node['zAZUsupply']:
                assert_equal(node['zAZUsupply'][denom], DecimalAmt(expected_zazu[denom]))
    ...
    def run_test(self):
        ...
        # Check AZU and zAZU supply at the beginning
        # ------------------------------------------
        # zAZU supply: 2 coins for each denomination
        expected_zazu_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # AZU supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zazu_supply)
        ...
        # Verify that AZU and zAZU supplies were properly updated after the spends and reorgs
        self.log.info("Check AZU and zAZU supply...")
        ...
        expected_zazu_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zazu_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zazu_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zazu_supply)
