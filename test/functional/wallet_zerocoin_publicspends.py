...
class ZerocoinSpendTest(AzusTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zazu_bal, azu_bal):
            zazu_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zazu_bal)
            azu_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], azu_bal)
            return zazu_bal, azu_bal
        ...
        zazu_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zazu_balance, 6666)
        ...
        zazu_balance, balance = check_balances(denom_2, zazu_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zazu_balance, balance = check_balances(denom_3, zazu_balance, balance)
        ...
        _, _ = check_balances(denom_1, zazu_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
