...
#include "zazu/zazuwallet.h"


CzAZUTracker::CzAZUTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzAZUTracker::~CzAZUTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAZUTracker::Init()
{
...
bool CzAZUTracker::Archive(CMintMeta& meta)
{
...
bool CzAZUTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAZUTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAZUTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAZUTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzAZUTracker::GetSerialHashes()
{
...
CAmount CzAZUTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAZUTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAZUTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAZUTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAZUTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAZUTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAZUTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAZUTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAZUTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAZUTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzAZUTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzAZUWallet* zAZUWallet)
{
    bool iszAZUWalletInitialized = (nullptr != zAZUWallet);
    CMintMeta meta;
    ...
    if (!iszAZUWalletInitialized)
        zAZUWallet = new CzAZUWallet(wallet);
    meta.isSeedCorrect = zAZUWallet->CheckSeed(dMint);
    if (!iszAZUWalletInitialized)
        delete zAZUWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzAZUTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAZUTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAZUTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAZUTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAZUTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAZUTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzazu from DB\n", __func__, listDeterministicDB.size());
    }
