...
CzAZUWallet::CzAZUWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAZUSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAZUSeed_deprecated()) {
                    LogPrintf("%s: Updated zAZU seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zazu seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zazu wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzAZUWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAZUCount(nCountLastUsed);
    else if (!walletdb.ReadZAZUCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAZUWallet::Lock()
{
    seedMaster.SetNull();
}

void CzAZUWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAZUWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAZU(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAZUWallet::LoadMintPoolFromDB()
{
...
void CzAZUWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAZUWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAZUWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zazuTracker->HasPubcoinHash(pMint.first)) {
...
bool CzAZUWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAZU(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    wallet->zazuTracker->Add(dMint, true);
...
void CzAZUWallet::SeedToZAZU(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzAZUWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAZUWallet::GenerateDeterministicZAZU(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAZUWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAZUWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzAZUWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
