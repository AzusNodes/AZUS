...
bool CLegacyZAzuStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZAzuStake from non zc-spend", __func__);
    ...
    *this = CLegacyZAzuStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zazu stake origin", __func__);

    // All good
    return true;
}

CLegacyZAzuStake::CLegacyZAzuStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZAzuStake::GetIndexFrom()
{
...
CAmount CLegacyZAzuStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZAzuStake::GetUniqueness() const
{
...
bool CLegacyZAzuStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zAZU stake block: height %d outside range", __func__, nHeight);
