...
bool CAzuStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CAzuStake from zerocoin spend", __func__);
...
bool CAzuStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CAzuStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CAzuStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CAzuStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CAzuStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CAzuStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CAzuStake::GetUniqueness() const
{
...
CBlockIndex* CAzuStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CAzuStake::ContextCheck(int nHeight, uint32_t nTime)
{
