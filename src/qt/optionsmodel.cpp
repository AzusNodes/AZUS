...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::AZU);
    ...
    if (settings.contains("nAnonymizeAzusAmount") || reset)
        SoftSetArg("-anonymizeazusamount", settings.value("nAnonymizeAzusAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in azus.cpp during shotdown to also remove the window positions
