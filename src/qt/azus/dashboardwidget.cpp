...
#include "qt/azus/dashboardwidget.h"
#include "qt/azus/forms/ui_dashboardwidget.h"
#include "qt/azus/sendconfirmdialog.h"
#include "qt/azus/txrow.h"
...
DashboardWidget::DashboardWidget(AZUSGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareAzu, "square-chart-azu");
    setCssProperty(ui->labelSquarezAzu, "square-chart-zazu");
    setCssProperty(ui->labelAzu, "text-chart-azu");
    setCssProperty(ui->labelZazu, "text-chart-zazu");
    ui->labelAmountZazu->setVisible(false);
    ui->labelSquarezAzu->setVisible(false);
    ui->labelZazu->setVisible(false);
    ...
    setCssProperty(ui->labelAmountAzu, "text-stake-azu-disable");
    setCssProperty(ui->labelAmountZazu, "text-stake-zazu-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &AZUSGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &AZUSGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &AZUSGUI::openFAQ);
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isAzu = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZAZU;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            azu = (pair.first != 0) ? pair.first / 100000000 : 0;
            zazu = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalAzu += pair.first;
            chartData->totalZazu += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesAzu.append(azu);
        chartData->valueszAzu.append(zazu);

        int max = std::max(azu, zazu);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->append(chartData->valuesAzu);
    set1->append(chartData->valueszAzu);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalAzu > 0 || chartData->totalZazu > 0) {
        setCssProperty(ui->labelAmountAzu, "text-stake-azu");
        setCssProperty(ui->labelAmountZazu, "text-stake-zazu");
    } else {
        setCssProperty(ui->labelAmountAzu, "text-stake-azu-disable");
        setCssProperty(ui->labelAmountZazu, "text-stake-zazu-disable");
    }
    forceUpdateStyle({ui->labelAmountAzu, ui->labelAmountZazu});
    ui->labelAmountAzu->setText(GUIUtil::formatBalance(chartData->totalAzu, nDisplayUnit));
    ui->labelAmountZazu->setText(GUIUtil::formatBalance(chartData->totalZazu, nDisplayUnit, true));
