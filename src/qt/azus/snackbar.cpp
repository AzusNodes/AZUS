...
#include "qt/azus/forms/ui_snackbar.h"
#include "qt/azus/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(AZUSGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &AZUSGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
