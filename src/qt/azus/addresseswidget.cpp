...
#include "qt/azus/forms/ui_addresseswidget.h"
#include "qt/azus/addresslabelrow.h"
#include "qt/azus/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(AZUSGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination azuAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(azuAdd, label.toUtf8().constData(),
