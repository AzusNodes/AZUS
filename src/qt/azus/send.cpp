...
#include "qt/azus/forms/ui_send.h"
#include "qt/azus/addnewcontactdialog.h"
#include "qt/azus/qtutils.h"
#include "qt/azus/sendchangeaddressdialog.h"
#include "qt/azus/optionbutton.h"
#include "qt/azus/sendconfirmdialog.h"
#include "qt/azus/myaddressrow.h"
#include "qt/azus/guitransactionsutils.h"
...
SendWidget::SendWidget(AZUSGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-azu");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination azuAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(azuAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(azuAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
