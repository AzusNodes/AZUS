...
const char * const AZUS_CONF_FILENAME = "azus.conf";
const char * const AZUS_PID_FILENAME = "azus.pid";
const char * const AZUS_MASTERNODE_CONF_FILENAME = "masternode.conf";
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AZUS";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AZUSParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "AZUSParams";
#else
    // Unix
    return pathRet / ".azus-params";
#endif
#endif
}
...
{
    fs::path pathConfigFile(GetArg("-conf", AZUS_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(GetArg("-mnconf", AZUS_MASTERNODE_CONF_FILENAME));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty azus.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override azus.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", AZUS_PID_FILENAME));
