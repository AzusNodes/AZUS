...
#if defined(NDEBUG)
#error "AZUS cannot be compiled without assertions."
#endif
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zAZU mint output in tx %s",
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (nHeight > nLast)   return 1     * COIN;
    if (nHeight > 900000)  return 0.95  * COIN;
    if (nHeight > 800000)  return 0.9   * COIN;
    if (nHeight > 700000)  return 0.85  * COIN;
    if (nHeight > 600000)  return 0.8   * COIN;
    if (nHeight > 500000)  return 0.75  * COIN;
    if (nHeight > 400000)  return 0.7   * COIN;
    if (nHeight > 300000)  return 0.65  * COIN;
    if (nHeight > 200000)  return 0.6   * COIN;
    if (nHeight > 150000)  return 0.55  * COIN;
    if (nHeight > 100000)  return 0.5   * COIN;
    if (nHeight > 50000)   return 0.45  * COIN;
    if (nHeight > 25000)   return 0.4  * COIN;
    if (nHeight > 1500)    return 0.35  * COIN;
    if (nHeight > 500)     return 0.2   * COIN;
    if (nHeight !=1)       return 0.1   * COIN;
    // Premine for masternodes at block 1
    return 100000 * COIN;
}

int64_t GetMasternodePayment(int64_t blockValue)
{
    return 0.9 * blockValue;
}
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZAZUModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
DisconnectResult DisconnectBlock(CBlock& block, CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    if (!UpdateZAZUSupplyDisconnect(block, pindex)) {
        error("%s: Failed to calculate new zAZU supply", __func__);
        return DISCONNECT_FAILED;
    }
...
{
    util::ThreadRename("azus-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    PublicCoinSpend publicSpend(params);
                    if (!ZAZUModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
    ...
    if (!UpdateZAZUSupplyConnect(block, pindex, fJustCheck)) {
        return state.DoS(100, error("%s: Failed to calculate new zAZU supply for block=%s height=%d", __func__,
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateAZUSupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zAZU serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Azus Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nAzusBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nAzusBadBlockBits)) {
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> azuInputs;
        std::vector<CTxIn> zAZUInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zAZUInputs.push_back(stakeIn);
            }else{
                azuInputs.push_back(stakeIn);
            }
        }
        const bool hasAZUInputs = !azuInputs.empty();
        const bool hasZAZUInputs = !zAZUInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasAZUInputs) {
                    ...
                    for (const CTxIn& azuIn : azuInputs)
                        if(azuIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            if(hasZAZUInputs) {
                for (const CTxIn& zAzuInput : zAZUInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zAzuInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zazu");
