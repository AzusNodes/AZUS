...
#include "zazu/zazumodule.h"
#include "zazu/zazuwallet.h"
#include "zazu/zazutracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_AZUS = 61000, // inherited version..
...
enum ZerocoinSpendStatus {
    ZAZU_SPEND_OKAY = 0,                            // No error
    ZAZU_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZAZU_WALLET_LOCKED = 2,                         // Wallet was locked
    ZAZU_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZAZU_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZAZU_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZAZU_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZAZU_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZAZU_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZAZU_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZAZU_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZAZU_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZAZU_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZAZU_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZAZU_SPENT_USED_ZAZU = 14,                      // Coin has already been spend
    ZAZU_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZAZU_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void doZAzuRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZAZU);
    ...
    bool CreateZAZUOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zPIV wallet
    std::unique_ptr<CzAZUTracker> zazuTracker{nullptr};
    void setZWallet(CzAZUWallet* zwallet);
    CzAZUWallet* getZWallet();
