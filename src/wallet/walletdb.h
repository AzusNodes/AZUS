...
public:
    ...
    bool WriteZAZUSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZAZUSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZAZUSeed_deprecated(uint256& seed);
    bool EraseZAZUSeed();
    bool EraseZAZUSeed_deprecated();

    bool WriteZAZUCount(const uint32_t& nCount);
    bool ReadZAZUCount(uint32_t& nCount);
