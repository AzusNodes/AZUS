...
{
    CDataStream serializedCoinSpend = ZAZUModule::ScriptSigToSerializedSpend(txin.scriptSig);
...
bool UpdateZAZUSupplyConnect(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    AssertLockHeld(cs_main);
...
bool UpdateZAZUSupplyDisconnect(const CBlock& block, CBlockIndex* pindex)
{
    AssertLockHeld(cs_main);
